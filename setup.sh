...
setup_hhh4b2tau() {
    ...
    #   - "HHH4B2TAU_", for features provided by the analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "HHH4B2TAU_" variables is documented below.
    #
    ...
    if [ "${HHH4B2TAU_SETUP}" = "1" ]; then
        >&2 echo "the HHH4b2tau analysis was already succesfully setup"
    ...
    #
    # global variables
    # (HHH4B2TAU = HHH4b2tau, CF = columnflow)
    #
    ...
    if [ "${CF_REMOTE_ENV}" != "1" ]; then
        ...
        cf_setup_interactive "${CF_SETUP_NAME}" "${HHH4B2TAU_BASE}/.setups/${CF_SETUP_NAME}.sh" || return "$?"
    fi
    ...
    if [ -e "${HHH4B2TAU_BASE}/.git" ]; then
        local m
        for m in $( ls -1q "${HHH4B2TAU_BASE}/modules" ); do
            cf_init_submodule "${HHH4B2TAU_BASE}" "modules/${m}"
        done
    fi
    ...
    export LAW_HOME="${LAW_HOME:-${HHH4B2TAU_BASE}/.law}"
    export LAW_CONFIG_FILE="${LAW_CONFIG_FILE:-${HHH4B2TAU_BASE}/law.cfg}"
...
main() {
    ...
    if setup_hhh4b2tau "$@"; then
        cf_color green "HHH4b2tau analysis successfully setup"
        return "0"
    else
        local code="$?"
...
# entry point
if [ "${HHH4B2TAU_SKIP_SETUP}" != "1" ]; then
    main "$@"
fi
